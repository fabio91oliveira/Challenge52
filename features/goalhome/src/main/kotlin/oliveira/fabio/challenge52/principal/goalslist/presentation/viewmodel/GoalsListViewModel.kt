package oliveira.fabio.challenge52.principal.goalslist.presentation.viewmodel
...
class GoalsListViewModel(
    ...
    val mutableLiveDataRemoved by lazy { MutableLiveData<Boolean>() }
    ...
    fun removeGoals() {
        launch {
            ...
            SuspendableResult.of<Int, Exception> { goalRepository.removeGoals(goalsToRemove) }
                .fold(
                    success = {
                        SuspendableResult.of<Int, Exception> {
                            weekRepository.removeWeeks(
                                weeksToRemove
                            )
                        }
                            .fold(success = {
                                mutableLiveDataGoals.value?.removeAll(goalWithWeeksToRemove)
                                mutableLiveDataRemoved.postValue(true)
                            }, failure = {
                                mutableLiveDataRemoved.postValue(false)
                            })
